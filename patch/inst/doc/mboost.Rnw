...
\editor{This is an extended version of a manuscript conditionally accepted for JMLR 2010-04-09}
...
\citep[per default via package \pkg{multicore},][]{PKG:multicore}. A more
...
are then passed to function \code{mboost()} for model fitting\footnote{The complete \textsf{R} code for
reproducing this example is given in the accompanying file \code{mboost-MLOSS.R}.}:
<<setup, echo = FALSE>>=
...
\begin{figure}
\begin{center}
<<mboost-bodyfat-plot-1, echo = FALSE, fig = TRUE, width = 7, height = 7>>=
### plot age and kneebreadth
layout(matrix(c(1, 2, 1, 3), nr = 2))
plot(cvm)
mtext(text = "(A)", side = 3, 1, adj = 1)
plot(model, which = "bols(anthro3b")
...
@
<<mboost-bodyfat-plot-2, echo = FALSE, fig = TRUE>>=
...
plot(model, which = "btree", newdata = nd, col.regions = col,
...
Secondly, an interface for parallel computing (based on \pkg{multicore}) is
