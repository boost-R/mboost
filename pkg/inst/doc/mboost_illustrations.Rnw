...
time2 <- system.time(west_lars <- lars(x, yfit, type = "lasso", use.Gram = FALSE))[1]
@
...
selection is computationally very efficient. As a comparison, computing all Lasso
solutions, using package \Rpackage{lars} \cite{efron04lars,pkg:lars} in
\textsf{R} (with 
\texttt{use.Gram=FALSE}), takes about
\Sexpr{formatC(round(time2, 1), digits = 2)} seconds.
