...
pd <- packageDescription("mboostDevel")
if (any(is.na(pd))){
    install.packages("mboostDevel", repos = "http://cran.at.r-project.org")
    pd <- packageDescription("mboostDevel")
}
...
\begin{center}
  ...
  Package mboost. \emph{Computational Statistics}\\
...
\textsf{R} via \R{vignette(package = "mboostDevel")}.
