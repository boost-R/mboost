...
load(system.file("cache/bodyfat_benchmarks.rda", package = "mboostDevel"))
aic <- AIC(bf_glm)
...
bf_bs <- glmboost(bsfm, data = bodyfat, center = FALSE, control = ctrl)
mstop(aic <- AIC(bf_bs))
@
...
%%load(system.file("cache/wpbc_benchmarks.rda", package = "mboostDevel"))
